...
- Before using a new library, inspect the relevant `package.json`. Prefer installed libraries such as Zod, TanStack Query, TanStack Form, Hono, Prisma, Expo, and `@oculus-business/contracts`.
...
Продукт: «Окулус Бизнес» — мобильное веб-приложение для управления ОРДИКС/OCULUS: CRM-канбан сделок, финансы (разовые и регулярные платежи, прогноз, runway), доска доработок и багов. Стек: vibe (Bun/Hono/Prisma/PostgreSQL + React/Vite webapp). Хостинг: Render free + Neon (решение владельца, см. CHECKLIST.md Deployment). Продуктовый модуль бэкенда: backend/src/modules/business. Язык интерфейса и коммуникации — русский.
