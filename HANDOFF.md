...
Цикл 2 (2026-09-12, план docs/ROADMAP-PLAN-R2.md, все стадии зачтены контролёром): дельты к прошлому месяцу на тайлах, колокольчик уведомлений (просрочки + комментарии, lastSeen), отчёт владельца (маржа по месяцам), контрагенты как сущность (справочник /app/companies, привязка сделок, автосоздание из карточки, Ctrl+K), № счёта у дебиторки, спарклайны на KPI дашборда, экспорт доски сделок CSV. Миграция 20260912190000 + бэкфилл.

Реализовано 2026-09-12 (см. docs/ROADMAP-PLAN.md): просрочка/причина отказа/«Оплачено»/CSV/«Мои»/цели месяца/PWA; отчёт воронки, движение MRR, поток 12 мес, дебиторка, история сделки, время в этапе; релизы+changelog, клиент у задачи, спринты, Ctrl+K, календарь, Telegram (задать TELEGRAM_BOT_TOKEN/TELEGRAM_CHAT_ID в env Render), ежедневный автобэкап в B2 (BACKUP_ENABLED=1, ключ backups/oculus-business-<дата>.json.gz, включая бэкап при холодном старте — free-tier засыпание не страшно).
