...
test('role navigation exposes only the current workspace', () => {
  ...
  // Продукт: нижняя навигация одинакова для всех ролей, админка — вторичная зона.
  expect(navigationItemsForRole('admin').every((item) => item.to.startsWith('/app'))).toBe(true)
  expect(homePathForRole('user')).toBe('/app')
  expect(homePathForRole('admin')).toBe('/app')
})
...
test('cross-role destinations resolve to the current role home', () => {
  ...
  expect(resolveRoleDestination('admin', '/app')).toBe('/app')
})
