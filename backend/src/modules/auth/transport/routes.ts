...
const refreshCookieName = 'oculus_business_refresh'
...
export function createAuthRoutes({ env, requireAuth, service }: CreateAuthRoutesOptions) {
  ...
  routes.openapi(cookieRegisterRoute, async (c) => {
    assertInviteCode(c.req.valid('json').inviteCode, env)
...
function assertInviteCode(provided: string | undefined, env: AppEnv) {
  const expected = env.SIGNUP_INVITE_CODE
  if (!expected) return
  if (provided !== expected) {
    throw new AppError(403, 'FORBIDDEN', 'Регистрация только по коду приглашения')
  }
}
